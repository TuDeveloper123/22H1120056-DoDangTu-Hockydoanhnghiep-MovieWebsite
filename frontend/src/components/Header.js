// File Path: frontend/src/components/Header.js
...
const Header = () => {
    ...
    const [search, setSearch] = useState(searchQuery.join(' '));
                        ...
                        {menuDisplay && user?._id && (
                            <div className='absolute right-0 top-full mt-2 w-48 bg-white rounded-md shadow-lg py-1 z-30 ring-1 ring-black ring-opacity-5'>
                                ...
                                <nav className='flex flex-col'>
                                    {/* *** SỬA ĐỔI CHÍNH Ở ĐÂY *** */}
                                    {/* Hiển thị link nếu vai trò là ADMIN hoặc STAFF */}
                                    {(user?.role === ROLE.ADMIN || user?.role === ROLE.STAFF) && (
                                        <Link
                                            // Điều hướng đến một trang mà Staff có quyền truy cập
                                            to={user.role === ROLE.ADMIN ? "/admin-panel/all-products" : "/admin-panel/ticket-status"}
